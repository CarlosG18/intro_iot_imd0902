...
DHT dht(DHTPIN, DHTTYPE); //inicaliza o sensor dht22
...
void setup() {
  ...
  //inicalizando o display = 12 colunas e 2 linhas
  lcd.begin(16, 2);
...
void loop() {
  ...
  if (mqtt_client.connected()){
    ...
    //mostrando a temperatura e umidade no display LCD
    lcd.clear(); //Apaga todos os caracteres do Display
    lcd.setCursor(0,0);//Marca o cursor na posição coluna 0, linha 0
    lcd.print("temp: ");// imprimindo na tela a temperatura
    lcd.setCursor(6,0);
    lcd.print(String(t).c_str());
    lcd.setCursor(0,1);//Marca o cursor na posição coluna 0, linha 1
    lcd.print("umidade: ");// imprimindo na tela a umidade
    lcd.setCursor(9,1);
    lcd.print(String(h).c_str());
    delay(3000);
  }
}
