...
unsigned long last_reset_time = 0; // variável para armazenar o timestamp do último reset diário
const unsigned long one_day_ms = 24 * 60 * 60 * 1000; // 24 horas em milissegundos

void reset_contador(){
    /* 
        Função para resetar o contador da porta quando passa o dia
    */
...
void setup() {
  ...
  // Leitura do último timestamp de reset diário armazenado na EEPROM
  EEPROM.get(4, last_reset_time);
...
void loop() {
  ...
  if (mqtt_client.connected()){
    ...
    bool send_contador_day = false;

    // Verificar se passou um dia desde o último reset
    unsigned long current_time = millis();
    if (current_time - last_reset_time >= one_day_ms) {
      contador_day = contador_open;
      resetDailyCounter();
      last_reset_time = current_time;
      EEPROM.put(4, last_reset_time);
      EEPROM.commit();
      send_contador_day = true;
    }
    ...
    if(send_contador_day){
        String payload = "{\"Luz\":" + String(fotoresistor_value)+",\"fimdecurso_left\":" + String(fimdecurso_left)+",\"fimdecurso_right\":" + String(fimdecurso_right)+",\"estado_armario\":" + estado_armario_s+",\"contador_day\":" + String(contador_day)+"}";
    }else{
